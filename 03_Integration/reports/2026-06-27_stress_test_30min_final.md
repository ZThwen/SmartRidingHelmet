...
2. **GNSS 天线未连接**：退避策略生效，但真实定位场景未覆盖
3. **心率/Voice 传感器未连接**：对应模块通过事件注入模拟，实际硬件响应待验证
...
| 心率传感器 | 未连接 | 心率/血氧查询通过事件注入模拟 |
| 语音模块 | 未连接 | 语音指令通过事件注入模拟 |
...
本次测试共发现并修复 8 个缺陷，涉及 9 个源文件：

| 严重度 | 缺陷 | 模块 | 修复方式 |
|:--:|------|------|------|
| 🔴 | AT 通道并发崩溃 | GNSS/Audio/SMS | 全局 AT_LOCK 互斥锁 |
| 🔴 | GNSS 无天线 900 次 AT 冲击 | GNSS | 三段式退避（正常→冷却→重试） |
| 🔴 | SMS 每次 spawn 线程碎片 | AlarmService | 持久线程 + ThreadSafeQueue |
| 🟡 | 温湿度永久放弃 | TempHumid | 5min 冷却期 + 一次自动复活 |
| 🟡 | SMS 冷却无锁 + 返回值未检查 | SMS | 冷却期防抖 + 返回值检查 |
| 🟡 | SMS 线程栈过小 | AlarmService | 8KB 线程栈（UCS2 中文编码需求） |
| 🟢 | AudioService 心跳位置不当 | AudioService | heartbeat 移到驱动守卫之前 |
| 🟢 | 离线模块无法定位诊断 | 压力测试脚本 | 添加逐模块详情诊断输出 |
...
| 项目 | 说明 | 优先级 |
|------|------|:--:|
| GNSS 天线连接 | 验证真实定位场景下的退避策略恢复（进入正常模式） | 高 |
| 心率传感器 (UART9) | 验证 UART9 初始化序列与 EC200U 的兼容性 | 高 |
| 语音模块 (ASRPRO) | 验证真实语音识别的 UART 通信稳定性 | 中 |
| BLE 真实手机连接 | 当前 BLE 为事件模拟，需真实手机配对待验证 GATT 通信 + MTU 协商 | 高 |
