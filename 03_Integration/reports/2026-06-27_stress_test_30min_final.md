...
| 指标 | 值 | 分析 |
|------|----|------|
| CPU 有效工作 | 3.5ms/轮 (26%) | 每轮主循环总耗时 13.5ms，其中 sleep(10ms) 占 74%，实际处理 23 个模块仅需 3.5ms。CPU 空闲 74%，有充足余量应对峰值负载。 |
| 单模块耗时 | 118μs | 23 模块平均每模块 tick() 仅 118 微秒。最重的模块（AHT20 I2C 82ms）通过采样间隔守卫跳过了绝大部分轮次，不影响整体效率。 |
| GC 回收 | 1775 次（每秒 1 次） | 测试主动每秒触发 gc.collect()，30 分钟累计 1775 次。内存从 119KB 降至 90KB 并保持稳定，证明 GC 有效回收且无泄漏。 |
| 调度频率 | 74.3 Hz | 13.5ms/轮 = 74 次/秒。远高于业务需求——GNSS 2s 一次、TTS 8s 一次——保证所有事件在 1 轮内响应，无积压。 |

**结论：框架在 74Hz 调度下仅消耗 26% CPU，单模块调度开销 118μs，设计高效且有充足峰值余量。**
