...
def scene_state_push(ctrl, bus, events):
    ...
    raw = send_ble_cmd(bus, "light_on")
    print("  发送: %s" % raw)
    print("  state 事件数: %d (应为1: 合并推送)" % len(events["state"]))
    for e in events["state"]:
        print("  回推: %s" % e)
    # 检查合并消息
    merged = events["state"][0] if events["state"] else {}
    if merged.get("t") == 7 and merged.get("m") == 1:
        print("  ✅ 灯光回推正确 (m=1=manual, b=%s)" % merged.get("b"))
    else:
        print("  ❌ 灯光回推不正确")
    if merged.get("v") == 5:
        print("  ✅ 音量回推正确 (v=%s)" % merged.get("v"))
    else:
        print("  ❌ 音量回推不正确")
    if merged.get("p") == 0:
        print("  ✅ 电源回推正确 (p=%s)" % merged.get("p"))
    else:
        print("  ❌ 电源回推不正确")
    wait_next()
