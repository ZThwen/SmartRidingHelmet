...
> **实现状态**：✅ **v1 已实现**（2026-05-18 E2E 测试通过）
> **负责人员**：郑皓文
...
| `EVENT_BATTERY_LOW` | `_on_battery_low(payload)` | PowerService 检测到低电量 | TTS 播报"当前电量不足" |
| `EVENT_BATTERY_CRITICAL` | `_on_battery_critical(payload)` | PowerService 检测到严重低电量 | TTS 播报（当前为 stub，待 PowerService 就绪后启用） |
...
| 事件 | 携带数据 | 发布时机 |
|:----|:--------|:--------|
| `EVENT_ALARM_TRIGGERED` | `{alarm_type, level, timestamp}` | 碰撞或 SOS 报警启动时 |
| `EVENT_ALARM_CANCELED` | `{duration, timestamp}` | 报警超时自动取消时 |

`EVENT_ALARM_TRIGGERED` 被 CloudService 订阅，用于立即推送报警到云端。
...
ctx:
  ...
  "alarm_type":    ""              # 当前报警类型: collision / sos / ""
  "alarm_level":   0               # 当前碰撞等级 1-3（仅碰撞）
  "alarm_start":   0               # 报警开始时间戳（用于超时判断）
  "alarm_timer":   None            # 超时定时器句柄（或靠 tick() 轮询）
...
2. **超时检查**：如果 `alarm_active == True` 且超过 30s，调用 `_cancel_alarm()`
3. 时间片控制：用 `check_interval_ms` 防止高频空转
...
| **等级联动** | Level 3 碰撞 → 发布 `EVENT_ALARM_TRIGGERED` 时 `alarm_type` 标注为 `sos` 级别，让 CloudService 按 SOS 处理 |
...
**解决**：所有 Device 调用前加 `if self.led:` / `if self.audio:` 保护。构造签名 `AlarmService(event_bus, led=None, audio=None)` 明确允许 None。
...
| 待测项 | 优先级 | 说明 |
|:------|:-----:|:------|
| 碰撞事件从 CollisionService 真实链路 | 高 | 当前通过 EventBus 手动注入，CollisionService 未开发 |
